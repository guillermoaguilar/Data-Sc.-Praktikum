...
data <- read.csv("C:\\Users\\miran\\Desktop\\Data-Sc.-Praktikum\\Data\\data.csv")
...
data_with_new_var <- no_protest0_df %>%
  mutate(clean_identity = case_when(str_detect(protesteridentity, regex("student", ignore_case = TRUE)) & 
   str_detect(protesteridentity, regex("farmer", ignore_case = TRUE)) ~ "farmers & students",
    ...
    str_detect(protesteridentity, regex("student", ignore_case = TRUE)) & 
    str_detect(protesteridentity, regex("doctor|nurse|medical", ignore_case = TRUE)) ~ "medical workers & students",
    str_detect(protesteridentity, regex("doctor|nurse|medical", ignore_case = TRUE)) ~ "medical workers",
    TRUE ~ "other"
  ))
...
clean_data <- data_with_new_var %>%
  select(year, country, protesterdemand1, protesterdemand2, protesterdemand3, protesterdemand4, 
         stateresponse1, stateresponse2, stateresponse3, stateresponse4, 
         stateresponse5, stateresponse6, stateresponse7, 
         gwf_democracy, clean_identity)
...
# Filter the data frame to keep only rows with "students" or "farmers & students" or "medical workers & students" in clean_identity
final_cleaned_df_students <- final_cleaned_df %>%
  filter(clean_identity %in% c("students", "farmers & students", "medical workers & students"))
